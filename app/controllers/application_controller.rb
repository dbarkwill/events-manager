class ApplicationController < ActionController::Base
  ...
  def dinner_delta
    @dinner_delta = ((ENV["MEALS_PURCHASED"].to_i - Customer.filter_by_rsvp(true).sum(:dinner_count).to_i) - (Customer.filter_by_checked_in(true).sum(:actual_dinner_count).to_i - Customer.filter_by_checked_in(true).sum(:dinner_count).to_i)).abs
  end
